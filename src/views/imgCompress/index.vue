<template>
  <div class="img-compress-box">
    ...
    <div class="img-compress-size-box">
      <sizeCompress ref="sizeCompressRef"></sizeCompress>
    </div>
    <!-- 图片上传盒子 -->
...
<script setup lang="ts">
  ...
  // 文件上传前触发
  const oldImgUrl = ref<string>('');
  const oldImgWidth = ref<number>(0); //  原石宽度
  const oldImgHeight = ref<number>(0); //  原石高度
  const windowURL = window.URL || window.webkitURL; //  兼容性处理
  ...
  // 压缩图片
  const newImgUrl = ref<string>('');
  const sizeCompressRef = ref<any>(null);
  const newImgWidth = ref<number>(0); // 压缩后宽度
  const newImgHeight = ref<number>(0); // 压缩后高度
  const toCompress = async () => {
    ...
    const res = await imageConversion.compress(oldFile.value, config); //  压缩
    const files = new window.File([res], oldFile.value.name, {
      type: res.type
    });
    newFile.value = files;
    newImgUrl.value = URL.createObjectURL(files); //  转为图片链接
    const img = new Image();
  ...
  const toDownload = () => {
    ...
    tempLink.download = newFile.value.name;
